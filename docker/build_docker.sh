...
if [ "$input_arg" == "isaac_sim_2022.2.1" ]; then
...
elif [ "$input_arg" == "isaac_sim_4.0.0" ]; then
    echo "Building Isaac Sim headless docker"
    dockerfile="isaac_sim.dockerfile"
    ...
    echo "Unknown Argument. Please pass one of [x86, aarch64, isaac_sim_2022.2.1, isaac_sim_2023.1.0, isaac_sim_2023.1.1, isaac_sim_4.0.0]"
    exit
fi

echo "${dockerfile}"

docker build --build-arg ISAAC_SIM_VERSION=${isaac_sim_version} -t curobo_docker:${image_tag} -f docker/${dockerfile} . 
